...
1. Ensure the kernel build host has at least one completed build under `~/kernels/<tag>/` with `Image.gz`, `modules.tar.xz`, and `config.stock` (run any of the kernel_build tests first if needed).
2. On the Fastpath host, clone/update this repo and `cd ~/arm_kernel_install_guide`.
3. Run `./scripts/pull_kernel_artifacts.sh --host <KERNEL_HOST>` with no additional flags (the script always connects as ubuntu, copies from `~/kernels`, and stores results under `~/kernels` locally).
